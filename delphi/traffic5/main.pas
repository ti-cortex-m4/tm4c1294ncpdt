...
const
  ...
  stPROBABILITY1:       string  = 'Выпадковасьць1';
  stPROBABILITY2:       string  = 'Выпадковасьць2';
  stPROBABILITY3:       string  = 'Выпадковасьць3';
...
begin
  Application.MessageBox(PChar(stOut),'Памылка', mb_IconHand + mb_Ok);
end;

procedure TfrmMain.WrnBox(stOut: string);
begin
  Application.MessageBox(PChar(stOut),'Увага', mb_IconWarning + mb_Ok);
end;

procedure TfrmMain.InfBox(stOut: string);
begin
  Application.MessageBox(PChar(stOut),'Iнфармацыя', mb_IconAsterisk + mb_Ok);
end;
...
// преобразование числа в строку с ведущими нулями слева
...
begin
  if Count > 0 then begin
    ...
    if (Timer1.Enabled) then begin Terminal('Выпадковасьць 2'); exit; end;
    ...
    if (updProbability1.Position = 0) or (p1 > updProbability1.Position) then
      ...
      Terminal('Выпадковасьць 1,%: ' + IntToStr(p1) + ' ' + IntToStr(updProbability1.Position));
