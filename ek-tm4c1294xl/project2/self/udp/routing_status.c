...
static err_t GetRouingStatusContent0(struct udp_pcb *pcb, struct pbuf *p, struct ip_addr *addr, uint port, uchar broadcast, const uint wIdx, const uchar u) {
  switch (wIdx) {
    ...
    case 7: return OutVersion(pcb,p,addr,port,broadcast);
    case 8: return OutBuff(pcb,p,addr,port,broadcast,BuffPrintF(szRowSU, szWatchdogReset, fWatchdogReset));

    case 9: return OutBuff(pcb,p,addr,port,broadcast,BuffPrintF(szHeaderS, "Modem"));
    ...
    case 14: return OutBuff(pcb,p,addr,port,broadcast,BuffPrintF(szHeaderS, "Kernel"));
...
err_t GetRouingStatusContent(struct udp_pcb *pcb, struct pbuf *p, struct ip_addr *addr, uint port, uchar broadcast) {
  ...
  switch (ibRoutingStatus) {
    ...
    default: return GetRouingStatusContent2(pcb,p,addr,port,broadcast,wIdx,u);
  }
}
