...
#ifndef SKIP_41
    case 41: SetNext(DEV_START_41P); break;
#endif
  }
...
void    NextProfile(void)
{
  ...
  {
    Clear(); sprintf(szLo+1,"прин€то: %04u",cwHouRead); DelayMsg();
  }
  ...
  if (cwHouRead == 0)
  {
    // запрещаем опрашивать другие каналы, принадлежащие текущему счЄтчику
    LoadCurrDigital(ibDig);
    for (c=0; c<bCANALS; c++)
    {
      LoadPrevDigital(c);
...
{
  NewLimits(); // переход границы со стороны счЄтчика
