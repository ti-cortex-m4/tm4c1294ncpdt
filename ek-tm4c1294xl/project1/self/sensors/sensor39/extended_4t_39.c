...
status  CntMonCanTariff39_Internal(uchar  ibMon, uchar  ibTariff)
{
  ...
  if (!tm2.fValid) return BadDigital(120+0);
  time tm = tm2.tiValue;
  ...
  Query39_DISC();
  if (Input39() != SER_GOODCHECK) return BadDigital(120+1);
...
status  ReadCntMonCanTariff39(uchar  ibMonAbs, uchar  ibTariff) // на начало мес€ца
{
  ...
  Query39_DISC();
  if (Input39() != SER_GOODCHECK) return BadDigital(120+3);

  return BadDigital(120+4);
}
