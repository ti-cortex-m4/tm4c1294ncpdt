...
slong64_ ReadRegisterScalerDLMS(void)
{
  ...
  if (!scaler.fValid) return GetSLong64Error1(Error39(30+4));

  ulong64_ unit = PopUnsignedValueDLSM();
  if (!unit.fValid) return GetSLong64Error1(Error39(30+5));
...
double2 ReadRegisterValueWithScaler39(const obis_t  obis, caller39*  pc)
{
  ...
  if (!value.fValid) return GetDouble2Error1(Error39(30+16));

  double2 scaler = ReadRegisterScaler39(obis, pc);
  if (!scaler.fValid) return GetDouble2Error1(Error39(30+17));
