...
// посылка запроса на открытие канала связи для сумматоров СЭМ-2
...
// чтение времени/даты для сумматоров СЭМ-2
...
uchar   i,j;
  ...
  // индекс получаса с опрошенного сумматора СЭМ-2
  j = tiDig.bHour*2 + tiDig.bMinute/30;

  // время на начало текущих суток
  tiDig.bHour   = 0;
  tiDig.bMinute = 0;
  ...
  {  
    ...
    if ((tiAlt.bYear  == tiCurr.bYear)  &&      // проверка для текущих суток
...
uchar   i;
  ...
  do 
  {
    ...
    {  
      ...
      // проверка на наличие брака для всех каналов
...
// переход на нужные сутки/месяц назад с помощь команд Esc l/Esc v
